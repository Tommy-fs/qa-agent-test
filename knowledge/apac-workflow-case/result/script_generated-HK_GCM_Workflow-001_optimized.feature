```gherkin
Feature: HongKong GCM Workflow - HK_GCM_Workflow-001
  Instruction Detail:
    1) Author: [Your Name]
    2) Workflow: HK GCM
  ...
  @hkgcm
  Scenario Outline: Verify HK GCM Workflow with THIRD PARTY PAYMENT=Yes and QC REQUIRE=True
    ...
    # ***************************************************************
    # Action: Open the application url and login as processing maker of HK Loans
    Given WebAgent open "$xxx systemApacLoginPage" url
    # Action: Login as processing maker
    When Login as "SopsM_HK"

    # Action: Indicate the below actions repository is on Instruction Tab
    Then WebAgent is on InstructionTab

    # Action: Switch Platform to HK Loans
    Then Switch Platform to "HK Loans"

    # Action: Create New Instruction(Mandatory)
    Then WebAgent click on createButton
    And WebAgent click on newInstructionItem
    And Wait 5 seconds

    # Action: Set fields value in MainSection(Mandatory)
    ...
    # Action: Set fields value in General Information Section(Mandatory)
    ...
    # Action: Set fields value in MATURITY DT Section(Mandatory)
    ...
    # Action: Set fields value in Loans Detail Section(Mandatory)
    ...
    # Action: Set fields value in Interest Rate,Cost Rate and Funding Section(Mandatory)
    ...
    # Action: Set fields value in Additional Remark Section(Mandatory)
    ...
    # Action: Set fields value in IMR Details Section(Mandatory)
    ...
    # Action: Switch to Operation Tab(Mandatory)
    Then WebAgent is on OperationTab

    # Action: Indicate the blow actions repository is on Operation Tab(Mandatory)
    Then WebAgent click on operationTab

    # Action: Set fields value in Operation Status Section(Mandatory)
    Then WebAgent type "PENDING REMARK <RNText>" into pendingRemarkTextarea
    And WebAgent check on kivTickbox

    # Action: Set fields value in Checklist Section(Mandatory)
    Then WebAgent click on 3ppBeneficiaryRadio

    # Action: Set fields value in Operation details Section(Mandatory)
    ...
    # Action: Set fields value in Booking Section(Mandatory)
    ...
    # Action: Perform the action -Create and Maker Submit(Mandatory)
    Then WebAgent click on createAndMakerSubmitButton

    # Action: Check the success message to confirm the instruction is created successfully and get the Instruction ID
    ...
    # Action: Login as Processing Checker of Hk Loans
    When Login as "SopsC HK"

    # Action: Indicate the below actions repository is on Operation Tab
    Then WebAgent is on OperationTab

    # Action: Open the URL of Instruction
    And WebAgent open "@instructionUrl.Value" url

    # Action: Check the warning message and wait for 5 seconds
    And WebAgent check on qcWarningMessage if exist
    And Wait 5 seconds

    # Action: Edit the instruction and click on Operation Tab
    Then WebAgent click on editButton
    Then WebAgent click on operationTab

    # Action: Set fields value in Checklist Section
    ...
    # Action: Set fields value in Operation details
    ...
    # Action: Set fields value in Approval Details
    ...
    # Action: Login as Quality Controller of HK Loans
    When Login as "SopsQC HK"

    # Action: Indicate the below actions repository is on Drawdown Tab
    ...
    # Action: Edit the instruction and click on Drawdown Tab
    Then WebAgent click on editButton
    Then WebAgent click on qcChecklistDrawdownTab

    # Action: Set fields value in Drawdown 0c Checklist section
    ...
    # Action: Perform the action -Complete Drawdown 0
    ...
    # Action: Perform the action - Submit to Payment
    Then WebAgent click on submitButton
    ...
    # Action: Login as Payment Maker of HK Loans
    When Login as "SopsPM HK"

    # Action: Indicate the below actions repository is on Payment Tab
    ...
    # Action: Edit the instruction and click on Payment Tab
    Then WebAgent click on editButton
    Then WebAgent click on paymentTab
    And Wait 5 seconds

    # Action: Set fields value in Payment Details Section
    ...
    # Action: Perform the action - Submit to Payment cheker
    Then WebAgent click on submitButton
    Then WebAgent click on submitToPaymentCheckerButton

    # Action: Check the success message confirm the action is successful
    And WebAgent see successMsg
    ...
    # Action: Login as Payment Checker of HK Loans
    ...
    # Action: Indicate the below actions repository is on Disbursement Tab
    ...
    # Action: Edit the instruction and click on Disbursement Tab
    Then WebAgent click on editButton
    Then WebAgent click on qcChecklistDisbursementTab
    And Wait 5 seconds

    # Action: Set fields value in Disbursment 0c Checklist section
    ...
    # Action: Perform the action - complete
    ...
    Examples:
      ...
      | GCM            | 2024-01-27 | 789012   | GHIJKL    | 2.3456 | 2000.00  |
      | GCM            | 2024-01-28 | 345678   | MNOPQR    | 3.4567 | 3000.00  |
      | GCM            | 2024-01-29 | 901234   | STUVWX    | 4.5678 | 4000.00  |
```
...
Feature: HK GCM Workflow
  Instruction Detail:
    ...
    3) Workflow: HK GCM
    4) Check Point: E2E
    5) Key Value: THIRD PARTY PAYMENT = No, QC REQUIRE = False

  @HK_GCM_Workflow-002
  Scenario Outline: Verify HK GCM Workflow with THIRD PARTY PAYMENT=<THIRD_PARTY_PAYMENT> and QC REQUIRE=<QC_REQUIRE>

    # Preconditions: User must have valid credentials for each role and the system is in a clean state.
    ...
    # Test Case ID: 2
    # Step 2: Create a new HK GCM Instruction with THIRD PARTY PAYMENT = <THIRD_PARTY_PAYMENT>, QC REQUIRE = <QC_REQUIRE>, other fields filled as required
    ...
    And Select "<QC_REQUIRE>" from qcRequireDropdownlist
    ...
    And WebAgent should see "Instruction created successfully"
    And WebAgent should see "CURRENT STATUS = KL LOANS OPS"
    And WebAgent should see "PROCESS STATUS = KL LOANS - PROCESSING-CHECKER"
    ...
    And WebAgent see successMsg
    And WebAgent should see "Action successful"
    And WebAgent should see "CURRENT STATUS = KL LOANS OPS"
    And WebAgent should see "PROCESS STATUS = KL LOANS - PROCESSING-CHECKER"
    # Sign Out the system
    Then sign Out
    ...
    And WebAgent open "@instructionUrl.Value" url
    And WebAgent should see instruction details

    # Test Case ID: 6
    # Step 6: Perform 'Submit to QC' action
    Then WebAgent click on editButton
    # Ensure that the edit button is clicked before modifying data
    Then WebAgent click on operationTab
    ...
    And WebAgent should see "Action successful"
    And WebAgent should see "CURRENT STATUS = KL LOANS OPS"
    And WebAgent should see "PROCESS STATUS = DRAWDOWN-QC"
    ...
    And WebAgent should see "Action successful"
    And WebAgent should see "CURRENT STATUS = KL LOANS OPS"
    And WebAgent should see "PROCESS STATUS = PROCESSING-CHECKER-QC CHECKED"
    ...
    And WebAgent should see "Action successful"
    And WebAgent should see "CURRENT STATUS = KL LOANS OPS"
    And WebAgent should see "PROCESS STATUS = PROCESSING-MAKER-MANUAL"
    # Sign Out the system
    Then Sign Out
    ...
    # Test Case ID: 16
    # Step 16: Verify available actions with THIRD PARTY PAYMENT = No, COMPLETED DATE = blank
    ...
    And WebAgent see successMsg
    And WebAgent should see "Action successful"
    And WebAgent should see "PROCESS STATUS = PAYMENT - MAKER"
    # Sign Out the system
    Then sign Out
    ...
    And WebAgent see successMsg
    And WebAgent should see "Action successful"
    And WebAgent should see "PROCESS STATUS = PAYMENT-CHECKER"
    # Sign Out the system
    Then sign Out
    ...
    And WebAgent see successMsg
    And WebAgent should see "Action successful"
    And WebAgent should see "CURRENT STATUS = COMPLETED"
    And WebAgent should see "PROCESS STATUS = COMPLETED"
    And WebAgent should see "COMPLETED DATE populated with today's date"
    # Sign Out the system
    Then sign Out
    ...
    # Test Case ID: 25
    # Step 25: Create a new HK GCM Instruction with THIRD PARTY PAYMENT = <THIRD_PARTY_PAYMENT>, QC REQUIRE = <QC_REQUIRE>, other fields filled as required
    Then WebAgent click on createButton
    And WebAgent click on newInstructionItem
    And Wait 5 seconds
    ...
    # Refactor steps 3-9 into a reusable scenario
    Then execute steps 3 to 9
    ...
    # Refactor steps 18-22 into a reusable scenario
    Then execute steps 18 to 22

    # Test Case ID: 29
    # Step 29: Verify available actions with THIRD PARTY PAYMENT = No, COMPLETED DATE != blank
    # Verify available actions
    Then Verify Action 'Submit to Payment' available
    ...
    Examples:
      | CUSTOMER_GROUP | THIRD_PARTY_PAYMENT | QC_REQUIRE |
      | GCM            | No                  | False      |

  Scenario: Reusable Steps 3 to 9
    # Steps 3-9: Maker Submit, Checker Login, Open Instruction, Submit to QC, QC Login, Open Instruction, Complete Drawdown QC
    # Login as KL LOANS OPS-PROCESSING-MAKER
    When Login as "KL LOANS OPS-PROCESSING-MAKER"
    # Indicate the below actions repository is on Instruction Tab
    Then WebAgent is on InstructionTab
    ...
    # Login as KL LOANS OPS-PROCESSING-CHECKER
    Given WebAgent open "$xxx systemApacLoginPage"url
    When Login as "KL LOANS OPS-PROCESSING-CHECKER"
    Then WebAgent is on InstructionTab
    Then Switch Platform to "HK Loans"
    ...
    # Perform 'Complete Drawdown QC' action
    Then WebAgent click on editButton
    # Ensure that the edit button is clicked before modifying data
    Then WebAgent click on qcChecklistDrawdownTab
  ...
  Scenario: Reusable Steps 18 to 22
    # Steps 18-22: Payment Maker Login, Open Instruction, Submit Payment Checker, Payment Checker Login, Open Instruction
    # Login as KL LOANS OPS-PROCESSING-PAYMENT MAKER
    Given WebAgent open "$xxx systemApacLoginPage"url
    When Login as "KL LOANS OPS-PROCESSING-PAYMENT MAKER"
    Then WebAgent is on PaymentTab
    Then Switch Platform to "HK Loans"
    ...
    # Perform 'Submit Payment Checker' action
    # Perform the action - Submit to Payment Checker
    Then WebAgent click on submitButton
    Then WebAgent click on submitToPaymentCheckerButton
    # Check the success message to confirm the action is successful
    ...
    # Login as KL LOANS OPS-PROCESSING-PAYMENT CHECKER
    Given WebAgent open "$xxx systemApacLoginPage"url
    When Login as "KL LOANS OPS-PROCESSING-PAYMENT CHECKER"
    Then WebAgent is on PaymentTab
    Then Switch Platform to "HK Loans"
